...
1. **Manifest at the root.** `workflow.awp.yaml` is the entry point. Without it, the runtime refuses to load the directory.
2. **One folder per agent.** Each agent lives in `agents/{agent_id}/` with its own `agent.awp.yaml`, implementation file, and optional `workflow/` subtree for prompts, schemas, preprocessors and skills. The folder name must match `identity.id` (validation rule R8).
3. **Runtime artifacts are write-only and excluded from packaging.** `workspace/` (memory), `runs/` (per-experiment execution traces, including nested A4 sub-run trees under `runs/<run_id>/iterations/NNN/delegations/<worker>/runs/<sub_run_id>/`), `data/state/` and `data/output/` are generated by the runtime and listed in the package exclusion rules.
