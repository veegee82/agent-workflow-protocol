...
class CritiqueEngine:
    ...
    def _build_critic_system_prompt(self, envelope: dict[str, Any]) -> str:
        ...
        # Rubric served by the outer-loop artifact registry (Phase A1). The
        # template carries a single ``{cat_list}`` placeholder and no other
        # format fields, so a plain ``str.format`` is safe. v0 default is
        # byte-identical to the previous hardcoded rubric.
        from awp.data.prompts import _artifact as _artifact_resolver

        return _artifact_resolver("critique_rubric").format(cat_list=cat_list)
