...
class TestCritiqueE2E:
    ...
    def test_critique_logged_to_disk(self, tmp_path: Path):
        ...
        Uses a mocked CritiqueEngine response so no live LLM call is
        required — this is a logging-pipeline test, not an LLM test.
        """
        ...
        # Mock the critique LLM call so the test is hermetic.
        fixed_envelope = CritiqueEnvelope(
            worker_id="w1",
            score=0.9,
            summary="Mocked critique for w1",
            defects=[],
        )
        with patch.object(
            runner._critique_engine,
            "critique_results",
            return_value=[fixed_envelope],
        ):
            delegation_results = [
                {
                    "worker_id": "w1",
