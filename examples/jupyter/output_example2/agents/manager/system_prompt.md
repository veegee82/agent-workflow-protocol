...
- **data** (dataframe) — file: `inputs/data.csv`
  Preview: DataFrame 100 rows x 4 cols
  Shape: 100 rows x 4 cols
  Columns: date (datetime64[us]), revenue (float64), users (int64), region (str)
- **config** (dict) — file: `inputs/config.json`
  Content: ```json
  {"target_metric": "revenue", "threshold": 0.8, "regions_of_interest": ["EU", "US"]}
  ```
- **context** (string)
  Content: We are preparing a quarterly business review for stakeholders.
