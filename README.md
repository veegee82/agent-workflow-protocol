...
- :sparkles: **Refinement Mode — task-local inference-compute scaling** — `awp refine <seed_run>` iteratively improves a completed run's deliverable using critique defects, gate rejections, and eval deltas as a deterministic gradient. Orthogonal to the outer loop (θ stays frozen; only y moves). Budget halves per iteration; stops on regression, plateau, wall-time cap, or empty gradient (R36). Winning iteration is hard-linked into `<seed>/BEST/`. *[Learn more](docs/refinement.md)*
