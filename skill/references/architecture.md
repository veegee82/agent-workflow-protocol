...
Manager decides: DELEGATE | COMPLETE | FAIL
    ...
    |       2. Workers execute in parallel (fan-out) in sandboxes
    |       3. Critique: analyze worker outputs for defects, trigger repairs
    |       4. Two-tier validation (deterministic + LLM semantic)
    |       5. Evaluation: score results, threshold-based retry/accept/fail
    |       6. Stall detection (confidence delta over window)
    |       7. Update rolling summary + check budget limits
    |       8. Next iteration -> back to Manager
    |
