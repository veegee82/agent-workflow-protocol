    ...
    def _code_execute(self, *, code: str, timeout: int = 120) -> dict[str, Any]:
        ...
        if self._workflow_dir:
            ...
            preamble = (
                ...
                # Standard library preamble so LLM code doesn't have to
                # remember to import these in every snippet. These are the
                # modules workers reach for most often; keeping them
                # pre-imported closes a whole class of NameError leaks
                # from LLM-written code.execute snippets.
                f"import json\n"
                f"import pathlib\n"
                f"import re\n"
                f"import math\n"
                ...
                f"# IMPORTANT: the persistent executor reuses the subprocess across\n"
                f"# code.execute calls, so _builtins.open may already point to a\n"
                f"# previous _safe_open. Binding _orig_open to the current wrapper\n"
                f"# produces an infinite wrap chain (_safe_open -> _safe_open ...)\n"
                f"# whose deepest fh gets closed on first __exit__, which then\n"
                f"# surfaces on the next call as 'I/O operation on closed file.'\n"
                f"# Fix: always capture the REAL builtin via a sentinel attribute\n"
                f"# on _builtins, installed exactly once.\n"
                f"if not hasattr(_builtins, '_awp_real_open'):\n"
                f"    _builtins._awp_real_open = _builtins.open\n"
                f"_orig_open = _builtins._awp_real_open\n"
                ...
                f"        self._mode = mode\n"
                f"        self._args = args\n"
                f"        self._kwargs = dict(kwargs)\n"
                f"        self._fh = _orig_open(path, mode, *args, **kwargs)\n"
                f"    def write(self, data):\n"
                f"        if isinstance(data, (bytes, bytearray, memoryview)) and 'b' not in self._mode:\n"
                f"            self._fh.close()\n"
                f"            new_mode = self._mode.replace('t', '') + 'b'\n"
                f"            kw = {{k: v for k, v in self._kwargs.items() if k not in ('encoding', 'errors', 'newline')}}\n"
                f"            self._fh = _orig_open(self._path, new_mode, *self._args, **kw)\n"
                f"            self._mode = new_mode\n"
                f"        return self._fh.write(data)\n"
                f"    def __getattr__(self, name):\n"
                f"        return getattr(self._fh, name)\n"
                f"    def __enter__(self):\n"
                f"        return self\n"
                f"    def __exit__(self, *exc):\n"
                f"        return self._fh.__exit__(*exc)\n"
                f"    def __iter__(self):\n"
                f"        return iter(self._fh)\n"
                f"def _safe_open(path, mode='r', *args, **kwargs):\n"
                f"    p = str(path)\n"
                f"    m = str(mode)\n"
                f"    if any(x in m for x in ('w', 'a', 'x')):\n"
                f"        parent = _os.path.dirname(p)\n"
                f"        if parent and not _os.path.isdir(parent):\n"
                f"            _os.makedirs(parent, exist_ok=True)\n"
                f"    # Only wrap text-mode writes; leave reads and binary untouched\n"
                f"    if 'b' not in m and any(x in m for x in ('w', 'a', 'x')):\n"
                f"        return _AWPFileProxy(path, m, args, kwargs)\n"
                f"    return _orig_open(path, mode, *args, **kwargs)\n"
                f"# Install the wrapper exactly once. Marking via a sentinel\n"
                f"# attribute guards against repeated re-wrapping across\n"
                f"# persistent-executor calls.\n"
                f"if not getattr(_builtins.open, '_awp_wrapped', False):\n"
                f"    _safe_open._awp_wrapped = True\n"
